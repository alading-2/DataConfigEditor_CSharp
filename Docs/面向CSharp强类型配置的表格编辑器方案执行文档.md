...
该计划保留完整路线，但当前代码已不再停留在 Phase 1-4。最新已落地的一批可验收功能包括：

- 工作区隐藏规则。
- 表格布局统一与设置面板重构。
- 基础搜索、排序、清除筛选。
- 列级筛选入口：enum 多选、数值比较、文本筛选。
- 源码解析基础上接入 DLL 类型元数据补强，用于列类型、enum 候选项和显示值格式化。

后续继续按本文后半段的筛选、类型系统、编辑与写回阶段推进，但应以“当前实现状态”为起点更新勾选项和验收记录。
