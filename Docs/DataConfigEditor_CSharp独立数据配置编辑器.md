...
完整表格产品形态、设置面板、筛选/排序/批量编辑/保存回写等后续计划，以 [面向 C# 强类型配置的表格编辑器完善计划](./面向CSharp强类型配置的表格编辑器完善计划.md) 为准。
...
- 右侧表格容器填满可用区域。
- 表格左上角使用固定边距，不因文件行数/列数不同而漂移。
- 表头固定在顶部，实例名列固定在左侧。
- 横向滚动条贴近表格区域底部。
- 空表、诊断视图、加载失败视图使用同一套内容锚点。

当前原型中某些表格显示得过于靠上，这是布局缺陷，不是配置数据差异。后续应按完善计划中的 Phase 1 修正。
...
左侧目录树需要参考 VS Code 的资源管理器体验：

- 默认隐藏 `bin/`、`obj/`、`.godot/`、`.git/`、`.idea/`、`.vscode/`、`.history/`、`.superpowers/`、`*.uid`。
- 支持用户隐藏某个文件夹或文件。
- 支持“显示隐藏项”开关，隐藏项以灰色显示。
- 隐藏规则只影响 DataConfigEditor，不修改 `.gitignore`。

### 3.8 设置面板

当前工具栏下拉设置只是临时方案。正式版本需要设置面板：

- 分类显示应用、工作区、表格、编辑、诊断设置。
- 支持“应用”“确定”“取消”“恢复默认”。
- 点击“应用”立即预览当前表格效果。
- 点击“取消”恢复进入设置前的状态。
- 隐藏规则、行高、字体、列宽、表头、冻结列等都在面板中统一管理。
